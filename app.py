...
def geocode_place(place_name, country_code_hint=None):
    place_name = clean_city_name(place_name)
    if not place_name:
        return None

    base_name = re.sub(r"\([^)]*\)", " ", place_name)
    base_name = re.sub(r"\s+", " ", base_name).strip(" ,-") or place_name
    main_name = base_name.split(",", 1)[0].strip().casefold()
    qualifiers = [part.strip().casefold() for part in base_name.split(",")[1:] if part.strip()]
    ...
    try:
        ...
        if country_code_hint:
            searches.append((base_name, country_code_hint))
        searches.append((base_name, None))
        if place_name.casefold() != base_name.casefold():
            searches.append((place_name, country_code_hint))

        seen_coordinates = set()
        for query, restricted_country in searches:
            locations = geolocator.geocode(
                ...
                namedetails=True,
                language="en",
                country_codes=restricted_country,
                timeout=12
            ) or []
            for location in locations:
                coordinate_key = (round(float(location.latitude), 5), round(float(location.longitude), 5))
                ...
                locality_names = [
                    ...
                    details.get("municipality"), details.get("county"),
                    location.raw.get("namedetails", {}).get("name")
                ]
                locality_names = [str(name).strip().casefold() for name in locality_names if name]
                importance = float(location.raw.get("importance") or 0)
                score = importance * 100
                if main_name in locality_names:
                    score += 120
                elif main_name and main_name in display_name.casefold():
                    score += 45
                score += sum(25 for qualifier in qualifiers if qualifier in display_name.casefold())
                if country_code_hint and details.get("country_code", "").casefold() == country_code_hint.casefold():
                    score += 35
                candidates.append((score, location, details, display_name))

        if candidates:
            _, location, details, display_name = max(candidates, key=lambda candidate: candidate[0])
            return {
                "name": base_name,
                ...
                "details": details,
                "country_code": details.get("country_code", "").casefold()
            }
    except Exception:
        return None
...
def get_route_data(source_city, destination, travel_option):
    ...
    source_country_code = (source_location or {}).get("country_code")
    destination_location = geocode_place(destination_name, source_country_code)
    ...
    def direct_route_result():
        ...
        return {
            ...
            "destination_country_code": destination_location.get("country_code"),
            "is_detailed": False
        }
    ...
    try:
        ...
        routed_distance_km = summary.get("distance", 0) / 1000
        unreasonable_route_limit = max(direct_distance * 3, direct_distance + 750)
        if routed_distance_km <= 0 or routed_distance_km > unreasonable_route_limit:
            return direct_route_result()
...
@st.cache_data(ttl=86400, show_spinner=False)
def reverse_geocode_stop(latitude, longitude, expected_country_code=None):
    try:
        ...
        stop_country_code = address.get("country_code", "").casefold()
        if expected_country_code and stop_country_code != expected_country_code.casefold():
            return None
        place_name = (
            address.get("city")
            or address.get("town")
...
def route_overnight_stops(route_data, stop_count):
    ...
    stops = []
    seen = set()
    source_country_code = route_data.get("source_country_code")
    destination_country_code = route_data.get("destination_country_code")
    expected_country_code = (
        source_country_code
        if source_country_code and source_country_code == destination_country_code
        else None
    )
    for stop_number in range(1, stop_count + 1):
        ...
        stop_name = reverse_geocode_stop(latitude, longitude, expected_country_code)
...
def estimate_flight(source, destination, members=1):
    ...
    destination_location = geocode_place(destination, (source_location or {}).get("country_code"))
    same_country = (
...
def estimate_trip_distance(source_city, destination):
    ...
    destination_location = geocode_place(destination, (source or {}).get("country_code"))
