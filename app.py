...
def get_destination_images(destination):
    ...
    url = f"https://api.pexels.com/v1/search?query={destination}&per_page=9"
...
def display_map(destination_name):
    try:
        ...
        if location:
            ...
            st.write("Detected:", location.address)
...
# ---------------- LOGIN / REGISTER ----------------
menu = st.sidebar.radio("Account", ["Login", "Register"])
...
if st.session_state.logged_in:
    ...
    cursor.execute(
        "SELECT destination FROM trips WHERE username=?",
