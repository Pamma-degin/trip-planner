...
def extract_voice_trip_fields(transcript):
    ...
    source_city, destination, days, interests, travel_option, stay_type.
    travel_option must be one of Bus, Train, Flight, Personal Car, Bike.
    stay_type must be one of Hostel, Budget Hotel, Comfort Hotel, Premium Hotel.
    ...
    if values.get("stay_type") in ("Hostel", "Budget Hotel", "Comfort Hotel", "Premium Hotel"):
        fields["stay_type"] = values["stay_type"]
    return fields
...
if st.session_state.chrome_menu_open:
    with st.sidebar.container(border=True):
        ...
        if hasattr(st, "page_link"):
            ...
            for page_path, page_label in optional_pages:
                if os.path.isfile(os.path.join(app_directory, page_path)):
...
# ---------------- TRIP PLANNER PAGE ----------------
    ...
    with col2:
        days = st.number_input("📅 Days", 1, 30, step=1, key="trip_days")
        ...
        if voice_hash and voice_hash != st.session_state.processed_voice_hash:
            if st.button("Use Voice Details", key="apply_voice_details"):
                try:
                    ...
                    st.error(f"Voice input could not be processed: {error}")
        ...
        if destination:
            with st.spinner("Generating your plan..."):
                try:
                    ...
                    Respect the selected travel and stay options. Do not invent exact live prices, schedules, hotel availability, or opening hours.
                    Begin with a short "Getting there" section for the selected travel option.
                    ...
                    st.error(f"Plan generation failed: {error}")
                else:
...
# ---------------- EXTRA FUNCTIONS ----------------
@st.cache_data(ttl=21600, show_spinner=False)
def google_places_search(search_query, limit=5):
...
@st.cache_data(ttl=21600, show_spinner=False)
def openstreetmap_places(destination, place_kind, limit=5):
    location = geocode_place(destination)
    if not location:
        return []

    if place_kind == "hotel":
        selectors = f'nwr(around:30000,{location["latitude"]},{location["longitude"]})["tourism"~"hotel|hostel|guest_house|motel"];'
    else:
        selectors = "\n".join([
...
def get_real_hotels(destination):
    google_results = google_places_search(f"hotels and hostels in {clean_city_name(destination)}", 5)
    osm_results = openstreetmap_places(destination, "hotel", 5) if len(google_results) < 5 else []
...
if page == "✈ Trip Planner":
    ...
    if total_budget > 0:
        ...
        if source_city and destination:
            current_budget_query = (
                ...
                members, travel_option, stay_type, budget, total_budget, currency
            )
    ...
    if st.session_state.response and result_destination:
        ...
        with col1:
            ...
            hotels = get_real_hotels(result_destination)
            ...
            if hotels:
                ...
                st.info("No verified hotel listings were found nearby.")
